...
I figured, "why not build such a kitchen-sink, but with in-between steps?" A mod pack, but it's a fork tree. I thought about this and realised that not only will this give players the freedom to choose exactly _how_ much they want to alter or expand the game, but also give us a way easier time maintaining these packs as when we update an upstream pack (i.e. Optimizations), those changes all trickle down through the forks, syncing from the upstream.
...
From the ground up, the game should run fast. That's why at the foundation we are first working on [[02 Packs/1. Lucent Optimizations/index|Lucent Optimizations]].
